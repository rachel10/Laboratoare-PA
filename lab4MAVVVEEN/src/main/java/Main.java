...
//                .forEach(hospital -> System.out.print(hospital.getName() + " "));
        ...
        Map<Resident, Hospital> solution = matching.algorithm();

        solution.keySet().forEach(resident -> System.out.println(resident.getName()+" -> "+solution.get(resident)));
        ...
        System.out.println();
        System.out.println("Problema generata:");
        System.out.println();
        ProblemGenerator problemGenerator= new ProblemGenerator();

        Problem problem1=problemGenerator.Generator();

        Matching matching1=new Matching(problem1);
        Map<Resident, Hospital> solution1 = matching1.algorithm();
        solution1.keySet().forEach(resident -> System.out.println(resident.getName()+" -> "+solution1.get(resident)));
       // problem1.getHospitals().forEach(hospital -> System.out.print(hospital.getName()+" "));
        if(matching1.isStable())
        {
            System.out.println("Matching is stable!");
        }
        else {
